...
ZurkMapsAVHonor = ZurkMapsAVHonor or {}

local Honor = ZurkMapsAVHonor
...
function Honor.GetBreakpointCelebrationSnapshot()
    local rank, progress = GetCurrentRankAndProgress()
    local hk, honor = 0, 0
    if type(GetPVPThisWeekStats) == "function" then
        local ok, valueHK, valueHonor = pcall(GetPVPThisWeekStats)
        if ok then
            hk = tonumber(valueHK) or 0
            honor = tonumber(valueHonor) or 0
        end
    end
    return {
        rank = rank,
        ...
        hk = hk,
        currentHonor = honor,
        milestones = BuildMilestones(rank, progress),
    }
end
...
function Honor.Create(parentFrame, addonFrame, mapHeight, config)
    ...
    local bar = CreateFrame("Frame", "ZurkMapsAVHonorBar", parentFrame, BackdropTemplateMixin and "BackdropTemplate" or nil)
    bar:SetWidth(15)
